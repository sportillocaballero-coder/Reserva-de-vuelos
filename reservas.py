...
def leerReservasJson():
    ...
    try:
        ruta = os.path.join(os.path.dirname(__file__), "datos", "reservas.json")
...
def guardarReservas(reservas):
    """Guarda las reservas en el archivo JSON"""
    return cargarArchivo(reservas, "reservas.json")
...
def reservarVuelo(usuario):
    ...
    try:
        ...
        print(f"Error al procesar el ID del vuelo: {e}")
        return
    ...
    try:
        cant = input("Cantidad de asientos: ").strip()
        if not cant:
        ...
        print(f"Error al procesar la cantidad: {e}")
        return
    ...
    for n in range(cant):
        while True:
            try:
                asiento_str = input(f"Elija asiento #{n+1} (ejemplo A1, B3): ").strip()
                if not asiento_str:
                    print("ERROR: el asiento no puede estar vacio")
                    continue
                asiento_str = asiento_str.upper()
                ...
                print("Error: formato de asiento invalido")
                continue
            except Exception as e:
                print(f"Error al procesar el asiento: {e}")
                continue
...
def historialReservas():
    """
    Objetivo: Mostrar el historial completo de reservas de todos los clientes
    Parametros: Nada
    Retorna: Nada
    """
    reservas = leerReservasJson()
    ...
    for reserva in reservas_ordenadas:
        ...
        id_reserva = reserva.get('id', 'N/A')
        vuelo_id = reserva.get('vuelo', 'N/A')
        cantidad = reserva.get('cant', 0)
        asientos = reserva.get('asientos', [])
        estado = reserva.get('estado', 'pendiente')
        
        asientos_str = ", ".join(asientos) if asientos else "N/A"
        
        print(f" ID Reserva: {id_reserva:<8} | Vuelo: {vuelo_id:<8} | Cantidad: {cantidad}")
        print(f" Asientos: {asientos_str:<30} | Estado: {estado.upper()}")
        print()
        contador += 1
